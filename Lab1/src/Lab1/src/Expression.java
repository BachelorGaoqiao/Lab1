...
public class Expression {
    ...
    public String derivate(String variable) {
        ...
        ArrayList<NumericTerm> derivedResult = new ArrayList<NumericTerm>();// 求导结果多项式的NumericTerm形式表达
        ArrayList<Integer> tempPowers = new ArrayList<Integer>();           // 各变量幂指数暂存列表
    ...
    public String simplify(String assignments) {
        ...
        for(String fragments : assignments.split("[=| ]")) {                // 用等号与空格进行分割
            if (isVariable) {                                               // 若此处应为变量名
                if (valueArray.contains(fragments))
        ...
        ArrayList<NumericTerm> simplifiedResult = new ArrayList<NumericTerm>();// 化简结果多项式的NumericTerm形式表达
        for(NumericTerm originalTerm : compileResults) {                    // 对于原多项式中的每一项
            ArrayList<Integer> tempPowers = new ArrayList<Integer>(originalTerm.powers);
                                                                            // 各变量幂指数暂存列表
